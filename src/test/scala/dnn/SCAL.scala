...
// Tester.
class SCALCompTests(df: SCAL_NCycle[matNxN])
...
class FXSCALCompTests(df: SCAL_NCycle[FXmatNxN])
                   (implicit p: config.Parameters) extends PeekPokeTester(df) {
  poke(df.io.enable.valid, true)
  poke(df.io.enable.bits.control, true)
...
class FPSCALCompTests(df: SCAL_NCycle[FPmatNxN])
                     (implicit p: config.Parameters) extends PeekPokeTester(df) {
  poke(df.io.enable.valid, true)
  poke(df.io.enable.bits.control, true)
...
class SCALCompTester extends FlatSpec with Matchers {
  ...
  it should "Typ Compute Tester" in {
    chisel3.iotesters.Driver.execute(Array("--backend-name", "verilator", "--target-dir", "test_run_dir"),
      () => new SCAL_NCycle(NumOuts = 1, ID = 0, 4, "Max")(new matNxN(2))) {
      c => new SCALCompTests(c)
    } should be(true)

    chisel3.iotesters.Driver.execute(Array("--backend-name", "verilator", "--target-dir", "test_run_dir"),
      () => new SCAL_NCycle(NumOuts = 1, ID = 0, 4, "Mul")(new FXmatNxN(2,4))) {
      c => new FXSCALCompTests(c)
    } should be(true)
    chisel3.iotesters.Driver.execute(Array("--backend-name", "verilator", "--target-dir", "test_run_dir"),
      () => new SCAL_NCycle(NumOuts = 1, ID = 0, 4, "Mul")(new FPmatNxN(2, t = FType.M))) {
