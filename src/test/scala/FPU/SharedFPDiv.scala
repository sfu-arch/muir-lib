...
class SharedFPUTests(c: SharedFPU)(implicit p: Parameters) extends PeekPokeTester(c) {
  ...
  poke(c.io.InData(0).bits.data_a, 0x6C00.U)
  poke(c.io.InData(0).bits.data_b, 0x4C00.U)
  poke(c.io.InData(0).bits.sqrt, 0.U)
  poke(c.io.InData(0).valid, 1.U)
  poke(c.io.InData(1).valid, 1.U)
  poke(c.io.InData(1).bits.data_a, 0x6C00.U)
  poke(c.io.InData(1).bits.data_b, 0x4C00.U)
  poke(c.io.InData(1).bits.sqrt, 1.U)
